...
```{r}
df.file <- data.frame(file=unique(df.summ00$file),var.name=c("Obs","Original","Update WDM","Update WDM-Sim"))
df.summ01 <- merge(df.summ00,df.file)
df.period <- data.frame(period=unique(df.summ00$period),long.per=c("Sim-period","Summer","Winter","Storm"))
...
```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="wtr",c("var.name","vol.ac.ft.sum")],
      format="markdown",row.names=FALSE)
```

```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="stm",c("var.name","vol.ac.ft.sum")],
      format="markdown",row.names=FALSE)
```


```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="ann",c("var.name","vol.ac.ft.mean")],
      format="markdown",row.names=FALSE)
```


```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="smr",c("var.name","vol.ac.ft.mean")],
      format="markdown",row.names=FALSE)
```

```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="wtr",c("var.name","vol.ac.ft.mean")],
      format="markdown",row.names=FALSE)
```

```{r, results='asis'}
x<-kable(df.summ01[df.summ00$period=="stm",c("var.name","vol.ac.ft.mean")],
      format="markdown",row.names=FALSE)
```
